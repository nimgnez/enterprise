                ...
                <td width="400" valign="top">
                    <form action="<%=path%>/manage/indexImg" method="post">
                        <table width="100%" border="0" cellpadding="8" cellspacing="0"
                               class="tableOnebor <c:if test="${e.id != 0}">formEdit</c:if>">
                            ...
                            <tr>
                                <td><b>图片名称</b>
                                    <input type="text" value="${e.title}" size="20" name="title"
                                           data-rule="图片名称:required;title;length[1~45];"
                                           id="title"/>
                                </td>
                            </tr>
                            <tr>
                                <td><b>图片</b>
                                    <input type="text" value="${e.pictureUrl}" name="pictureUrl" id="pictureUrl"
                                           data-rule="图片地址:required;pictureUrl;"/>
                                    <input type="button" name="filemanager" value="浏览图片" class="btn btn-primary"
                                           style="padding: 0px 12px;margin-top: -5px;"/>
                                    <c:choose>
                                        <c:when test="${!empty e.pictureUrl}">
                                            <a target="_blank"
                                               href="<%=SystemManage.getInstance().getSystemSetting().getImageRootPath()%>/${e.pictureUrl}">
                                                <img style="max-width: 50px;max-height: 50px;" alt=""
                                                     src="<%=SystemManage.getInstance().getSystemSetting().getImageRootPath()%>/${e.pictureUrl}">
                                            </a>
                                        </c:when>
                                    </c:choose></td>
                            </tr>
                            ...
                            <tr>
                                <td><b>排序</b>
                                    <input type="text" value="${e.orders}" name="orders" size="20"
                                           data-rule="排序:required;integer[+];orders;length[1~5];"
                                           id="orders"/>
                                </td>
                            </tr>
                            <tr>
                                    ...
                                    <select id="status" name="status" class="input-medium">
                                        <c:forEach var="entry" items="<%=status%>">
                                            <option value="${entry.key}"
                                                    <c:if test="${entry.key eq e.status}">selected="selected" </c:if>>${entry.value}</option>
                                        </c:forEach>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <a class="btn btn-default" style="padding: 2px 22px;margin-left: 50px;"
                                               href="selectList">取消</a>
                                            <button method="update" onclick="commit(this)" class="btn btn-info"
                                                    style="padding: 2px 22px;float: right;margin-right: 50px;">
                                                保存
                                            </button>
                                        </c:otherwise>
                                    </c:choose>
                        ...
                        <c:forEach var="item" items="${pager.list}">
                            <tr <c:if test="${e.id eq item.id}">class="active"</c:if>>
                                <td style="padding: 10px 10px;"><a
                                        href="<%=SystemManage.getInstance().getSystemSetting().getImageRootPath()%>${item.pictureUrl}"
                                        target="_blank"><img
                                        src="<%=SystemManage.getInstance().getSystemSetting().getImageRootPath()%>${item.pictureUrl}"
                                        width="200"/></a></td>
                                <td>${item.title}</td>
                                ...
                                <td align="center"><a href="toEdit?id=${item.id}">编辑</a> | <a
