    ...
    <div style="height:auto!important;height:550px;min-height:550px;">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="10" class="tableBasic">
            ...
            <tr>
                ...
                <td style="text-align: left;" colspan="3"><input type="text" value="${e.position}" name="position"
                                                                 size="40" id="position"
            ...
            <tr>
                ...
                <td style="text-align: left;" colspan="3">
                    <input type="text" value="${e.number}" name="number" size="4" id="number"
                           data-rule="人数:required;integer[+];">人
                </td>
            </tr>
            <tr>
                    ...
                    <select name="nature">
                        <c:forEach var="entry" items="<%=natures %>">
                            <option value="${entry.key}"
                                    <c:if test="${e.nature eq entry.key}">selected="selected" </c:if>>${entry.value}</option>
                        </c:forEach>
                    </select>
                </td>
                ...
                <td style="text-align: left;">
                    ...
                    <select name="experience">
                        <c:forEach var="entry" items="<%=experiences %>">
                            <option value="${entry.key}"
                                    <c:if test="${e.experience eq entry.key}">selected="selected" </c:if>>${entry.value}</option>
                        </c:forEach>
                    </select>
                    及以上
                </td>
                ...
                <td style="text-align: left;">
                    <select name="education">
                        <c:forEach var="entry" items="<%=educations %>">
                            <option value="${entry.key}"
                                    <c:if test="${e.education eq entry.key}">selected="selected" </c:if>>${entry.value}</option>
                        </c:forEach>
                    </select>
                    及以上
                </td>
            ...
            <tr>
                <td colspan="4" style="text-align: center;">
                    <c:choose>
                        <c:when test="${e.id == 0}">
                            <button method="insert" class="btn btn-info" style="padding:2px 15px;"
                                    onclick="return selectList(this)">
                                新增
                            </button>
                        </c:when>
                        <c:otherwise>
                            <button method="update" class="btn btn-info" style="padding:2px 15px;"
                                    onclick="return selectList(this)">
                                保存
                            </button>
                        </c:otherwise>
