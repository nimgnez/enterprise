                ...
                <select id="diffAreaLogin" name="diffAreaLogin">
                    <c:forEach var="entry" items="<%=types %>">
                        <option value="${entry.key}"
                                <c:if test="${e.diffAreaLogin eq entry.key}">selected="selected" </c:if>>${entry.value}</option>
                    </c:forEach>
                </select>
                <input type="text" value="${e.account}" class="input-medium search-query" name="account"
                       placeholder="管理员帐号"/>
